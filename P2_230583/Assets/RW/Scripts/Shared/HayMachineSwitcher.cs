...
public class HayMachineSwitcher : MonoBehaviour, IPointerClickHandler
{
    ...
    public void OnPointerClick(PointerEventData eventData) // called when this GameObject gets clicked
    {
        selectedIndex++; 
        selectedIndex %= Enum.GetValues(typeof(HayMachineColor)).Length; // modulo

        GameSettings.hayMachineColor = (HayMachineColor)selectedIndex;

        // Enable and disable the hay machine models based on the chosen machine color
